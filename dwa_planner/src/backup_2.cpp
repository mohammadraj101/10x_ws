...
class DwaPlanner : public rclcpp::Node{
    ...
    // private:
        ...
        double cost_obstacle(const traj&t)
        {
            ...
            auto cost_= 1.0 / (1.0 + exp(4*(min_dist-0.5)));   //a=6 & b=0.4 try this also
            return cost_;
        }
        ...
        double cost_heading_angle(const traj&t)
        {
            ...
            // heading_error = pow(heading_error/M_PI, 2 );

            double dist_weight = 1.0 / (1.0 + exp(goal_dist - 1.0));


            return heading_error*dist_weight;
        }
        ...
        double generate_cost(const traj&t)
        {
            ...
            // double bt=1.0 / (1.0 + exp(10.0 * (obstacle_error - 0.2)));
            // double al=1-bt;

            // double cost = al * head_error + bt * obstacle_error ;
