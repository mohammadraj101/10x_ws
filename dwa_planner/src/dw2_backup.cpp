...
const double SAFETY_DISTANCE = 0.25;

const double V_MIN =  0.07, V_MAX = 0.7;
...
class DwaPlanner : public rclcpp::Node{
    ...
    public:
        DwaPlanner()
            : Node("dwa_planner")
            {
                ...
                timer_ = this->create_wall_timer(75ms, std::bind(&DwaPlanner::apply_DWA,this));
    ...
    // private:
        ...
        void apply_DWA()
        {
            ...
            RCLCPP_INFO(this->get_logger(), "Time taken for calculations: %lld ms", calc_ms);

            if(best_index>=0)
            {
                auto vf=trajectories[best_index].v;
                auto wf=trajectories[best_index].w;
                ...
                if (goal_dist < 0.15) // 0.1 meters threshold
                {
                    RCLCPP_INFO(this->get_logger(), "Goal reached! Stopping robot. flag= {}",flag);
                ...
                RCLCPP_INFO(this->get_logger(), "Current Pose: x: %.3f, y: %.3f, vx: %.3f, w: %.3f    COST: %.4f", 
                pose_x, pose_y, curr_vx, curr_w,trajectories[best_index].cost);
        ...
        double cost_obstacle(const traj&t)
        {
            ...
            if (t.points.empty()||obstacles.empty()) {
                return 1.0; // High default cost if trajectory is empty
            }
            ...
            auto cost_= 1.0 / (1.0 + exp(4*(min_dist-0.5)));
            return cost_;
        }
