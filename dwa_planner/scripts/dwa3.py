...
V_MIN, V_MAX = -0.0, 0.9
W_MIN, W_MAX = -0.5,0.5
A_MIN, A_MAX = -12.5, 12.5
AL_MIN, AL_MAX = -7.5, 7.5
V_RES, W_RES = 0.1, 0.1
T, DT = 1, 0.05
...
class DwaPlanner(Node):
    def __init__(self):
        ...
        self.marker_pub_ = self.create_publisher(MarkerArray, '/waypoints', 10)
    ...
    def apply_DWA(self):
        ...
        self.prev_time = self.get_clock().now()#.to_msg().sec*0.1
        # self.curr_time = self.get_clock().now().to_msg().sec*0.1

        # count=0

        while(self.get_clock().now()-self.prev_time<=Duration(seconds=T)):
        ...
        final=self.get_clock().now()
        print(f"Time taken for 1 cycle:   {(final-start)}")
