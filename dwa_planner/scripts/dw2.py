...
class DwaPlanner(Node):
    ...
    def scan_callback(self, scan):
        obstacles_list = []  # Use a list instead of vstack for efficiency
        angle = scan.angle_min

        for r in scan.ranges:
            if scan.range_min < r < scan.range_max:
                x =  r * np.cos(angle)  # Corrected: Directly compute global position
                y =  r * np.sin(angle)
                obstacles_list.append([x+self.pose_x, y+self.pose_y])  # Append instead of vstack

            angle = (angle + scan.angle_increment) % (2 * np.pi)

        self.obstacles = np.array(obstacles_list)
        
        # Print min range value (sanity check)
        # print("First 350 scan.ranges:", min(scan.ranges[:350]))
        # print()

        # Check if obstacles exist before computing closest point
        # if self.obstacles.shape[0] > 0:
        #     # Compute distances (No need to subtract pose_x and pose_y again)
        #     dist = np.linalg.norm(self.obstacles - np.array([self.pose_x, self.pose_y]), axis=1)

        #     # Find index of closest obstacle
        #     min_index = np.argmin(dist)
        #     min_distance = dist[min_index]

        #     # Get (x, y) coordinates of the closest obstacle
        #     closest_obstacle = self.obstacles[min_index]

            # Print results
        #     print(f"MIN_DISTANCE: {min_distance:.3f} meters")
        #     print(f"Closest Obstacle at: x={closest_obstacle[0]:.3f}, y={closest_obstacle[1]:.3f}")
        ...
        for row in trajectories:
            ...
            for traj in row:
                ...
                if traj is not None:
                    ...
                    norm_cost_angle = sigmoid(cost_angle, k=2.0, c=0.0)
                    final_cost = ALPHA * norm_cost_vel + BETA*cost_obstacle + GAMMA *cost_angle #+exta_penalty#+ GAMMA * norm_cost_vel
    ...
    def apply_DWA(self):
        ...
        if (ii != -1 and jj != -1 and trajectories[ii][jj] is not None):
